...
GET_DATA_SCHEMA = vol.Schema(
    {**_TARGET_SCHEMA, vol.Optional("history_limit", default=50): vol.Coerce(int)}
)

GET_RECORD_SCHEMA = vol.Schema(
    {**_TARGET_SCHEMA, vol.Required(ATTR_SERVICE_RECORD_ID): vol.Coerce(int)}
)
...
DELETE_ITEM_SCHEMA = vol.Schema(
    {**_TARGET_SCHEMA, vol.Required("item_id"): vol.Coerce(int)}
)

UPDATE_TIRE_SCHEMA = vol.Schema(
    {
        **_TARGET_SCHEMA,
        vol.Required("tire_id"): vol.Coerce(int),
        vol.Optional("position"): vol.In(TIRE_POSITIONS),
        vol.Optional("brand"): cv.string,
        vol.Optional("model"): cv.string,
        vol.Optional("size"): cv.string,
        vol.Optional("current_tread_depth"): vol.Coerce(float),
        vol.Optional("purchase_cost"): vol.Coerce(float),
        vol.Optional("active"): cv.boolean,
        vol.Optional(ATTR_NOTES): cv.string,
    }
)

DELETE_TIRE_SCHEMA = vol.Schema(
    {**_TARGET_SCHEMA, vol.Required("tire_id"): vol.Coerce(int)}
)

UPDATE_BRAKE_SCHEMA = vol.Schema(
    {
        **_TARGET_SCHEMA,
        vol.Required("brake_id"): vol.Coerce(int),
        vol.Optional("condition"): vol.In(BRAKE_CONDITIONS),
        vol.Optional("pad_thickness"): vol.Coerce(float),
        vol.Optional("rotor_condition"): cv.string,
        vol.Optional(ATTR_NOTES): cv.string,
    }
)

DELETE_BRAKE_SCHEMA = vol.Schema(
    {**_TARGET_SCHEMA, vol.Required("brake_id"): vol.Coerce(int)}
)

DELETE_CATEGORY_SCHEMA = vol.Schema(
    {**_TARGET_SCHEMA, vol.Required("category_id"): vol.Coerce(int)}
)
...
def async_register_services(hass: HomeAssistant) -> None:
    ...
    async def get_data(call: ServiceCall) -> ServiceResponse:
        """Return one payload with everything the Lovelace card renders."""
        runtime = _resolve_runtime(hass, call)
        repository = runtime.repository
        limit = int(call.data.get("history_limit", 50))
        ...
        def _collect() -> dict[str, Any]:
            return {
                "records": [
                    record.to_dict()
                    for record in repository.list_service_records(
                        runtime.vehicle_id, limit=limit
                    )
                ],
                "categories": [
                    category.to_dict()
                    for category in repository.list_categories(runtime.vehicle_id)
                ],
                "item_names": repository.list_item_names(runtime.vehicle_id),
                "schedules": [
                    schedule.to_dict()
                    for schedule in repository.list_schedules(runtime.vehicle_id)
                ],
                "tires": [
                    tire.to_dict()
                    for tire in repository.list_tire_records(runtime.vehicle_id)
                ],
                "brakes": [
                    brake.to_dict()
                    for brake in repository.list_brake_records(runtime.vehicle_id)
                ],
                "analytics": repository.analytics(runtime.vehicle_id),
            }

        collected = await hass.async_add_executor_job(_collect)
        return {
            "vehicle": data.vehicle.redacted(),
            "vehicle_id": runtime.vehicle_id,
            "entry_id": runtime.coordinator.entry.entry_id,
            "current_mileage": data.current_mileage,
            "distance_unit": runtime.coordinator.distance_unit,
    ...
    async def get_service_record(call: ServiceCall) -> ServiceResponse:
        """Return one service record with its items and attachments."""
        runtime = _resolve_runtime(hass, call)
        record = await hass.async_add_executor_job(
            runtime.repository.get_service_record,
            int(call.data[ATTR_SERVICE_RECORD_ID]),
        )
        if record is None:
            raise ServiceValidationError(
                f"Unknown service record {call.data[ATTR_SERVICE_RECORD_ID]}"
            )
        return {"record": record.to_dict()}

    async def update_maintenance_item(call: ServiceCall) -> None:
        runtime = _resolve_runtime(hass, call)
        updates = {
            key: value
            for key, value in call.data.items()
            if key not in (ATTR_ENTRY_ID, ATTR_VEHICLE_ID, "item_id")
        }
        if not updates:
            raise ServiceValidationError("No fields supplied to update")
        if ATTR_CATEGORY in updates:
            await hass.async_add_executor_job(
                runtime.repository.add_category,
                runtime.vehicle_id,
                updates[ATTR_CATEGORY],
            )
        try:
            await _run(
                runtime,
                runtime.repository.update_maintenance_item,
                int(call.data["item_id"]),
                updates,
            )
        except RepositoryError as err:
            raise HomeAssistantError(str(err)) from err

    async def delete_maintenance_item(call: ServiceCall) -> None:
        runtime = _resolve_runtime(hass, call)
        await _run(
            runtime,
            runtime.repository.delete_maintenance_item,
            int(call.data["item_id"]),
        )

    async def update_tire_record(call: ServiceCall) -> None:
        runtime = _resolve_runtime(hass, call)
        updates = {
            key: value
            for key, value in call.data.items()
            if key not in (ATTR_ENTRY_ID, ATTR_VEHICLE_ID, "tire_id")
        }
        if not updates:
            raise ServiceValidationError("No fields supplied to update")
        try:
            await _run(
                runtime,
                runtime.repository.update_tire_record,
                int(call.data["tire_id"]),
                updates,
            )
        except RepositoryError as err:
            raise HomeAssistantError(str(err)) from err

    async def delete_tire_record(call: ServiceCall) -> None:
        runtime = _resolve_runtime(hass, call)
        await _run(
            runtime, runtime.repository.delete_tire_record, int(call.data["tire_id"])
        )

    async def update_brake_record(call: ServiceCall) -> None:
        runtime = _resolve_runtime(hass, call)
        updates = {
            key: value
            for key, value in call.data.items()
            if key not in (ATTR_ENTRY_ID, ATTR_VEHICLE_ID, "brake_id")
        }
        if not updates:
            raise ServiceValidationError("No fields supplied to update")
        try:
            await _run(
                runtime,
                runtime.repository.update_brake_record,
                int(call.data["brake_id"]),
                updates,
            )
        except RepositoryError as err:
            raise HomeAssistantError(str(err)) from err

    async def delete_brake_record(call: ServiceCall) -> None:
        runtime = _resolve_runtime(hass, call)
        await _run(
            runtime, runtime.repository.delete_brake_record, int(call.data["brake_id"])
        )
    ...
    async def delete_category(call: ServiceCall) -> None:
        ...
        await _run(
            runtime,
            runtime.repository.delete_category,
            int(call.data["category_id"]),
        )
    ...
    registrations: list[tuple[str, Any, Any, SupportsResponse]] = [
        ...
        (SERVICE_GET_DATA, get_data, GET_DATA_SCHEMA, SupportsResponse.ONLY),
        (
            SERVICE_GET_SERVICE_RECORD,
            get_service_record,
            GET_RECORD_SCHEMA,
